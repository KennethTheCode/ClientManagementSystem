        ...
        <div className='w-[98%] mx-auto bg-stone-800 h-[65vh] rounded-lg flex'>
            <div className='bg-stone-900  w-[13vh] h-[65vh] p-2 flex-col overflow-y-auto'>
                <div className='p-1 rounded  mx-auto w-[95%]'>
                    <p className='text-white text-center mb-1 flex items-center justify-center gap-2'><span class="material-symbols-outlined">group</span> CLIENTS</p>
                </div>
                {activities.map((activity, index) =>(
                <div key={activity._id || index} className='bg-stone-800 w-full h-[5vh]  mb-2 rounded text-white flex items-center justify-center text-center hover:bg-stone-700 transition-colors duration-100'><p>{activity.clientName}</p></div>
                ))}
                
            </div>
            <div className=' w-full h-[65vh] p-3 flex flex-col gap-3 grid grid-cols-5 overflow-y-auto'>
             {activities.map((activity, index) => (
                <div key={activity._id || index} className='bg-stone-900 border border-stone-700 w-[30vh] h-[35vh] rounded hover:bg-stone-950 transition-colors duration-100'>
                    <div className='w-[95%] h-12 flex justify-between items-center mx-auto p-2 border-b border-stone-700'>
                      ...
                      <div className='w-20 flex items-center justify-center'>
                        <Project_control activity={activity} userId={userId} onDelete={handleDeleteActivity}/>
                      </div>
                    </div>
                    <div className='w-full p-3'>
                      ...
                      <div className='w-full h-[10vh] p-1 flex overflow-y-auto'>
                        <p className='text-white'>{activity.projectDescription}</p>
                      </div>
                    </div>
                  </div>
                ))}
